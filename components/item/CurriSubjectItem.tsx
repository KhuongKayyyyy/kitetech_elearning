...
export default function CurriSubjectItem({ subject }: CurriSubjectItemProps) {
  const prerequisites =
    subject.PrerequisiteSubjects?.slice(0, 2).map(
      (prereq: any) => prereq.SubjectName
    ) || [];
  const hasPrerequisites =
    subject.HasPrerequisite && subject.PrerequisiteSubjects?.length > 0;

  const prerequisiteSubjects = prerequisites.map((prereq: string) => {
    const subject = FakeData.getSubjects().find((s) => s.subjectId === prereq);
    return subject;
  });
    ...
    <TooltipProvider>
      <Tooltip>
            ...
            <div className='relative z-10 space-y-4'>
              ...
              <div className='space-y-1'>
                ...
                {subject.SubjectName_EN &&
                  subject.SubjectName_EN !== subject.SubjectName && (
              ...
              {subject.CourseTypeName && (
                <div
                  ...
                  style={{
                    backgroundColor:
                      subject.CourseTypeColorBackground || "#f8fafc",
        ...
        {hasPrerequisites && (
          <TooltipContent
            side='top'
            className='max-w-sm p-0 border-0 shadow-2xl bg-white/95 backdrop-blur-md'>
